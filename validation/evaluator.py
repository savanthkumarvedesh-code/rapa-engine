...
from data.db import get_cpi_benchmarks, get_all_index_records, DB_PATH
...
def evaluate_cpi_benchmark_tracking(
    ...
    if len(index_records) < 2:
        ...
        return {
            ...
            "official_mospi_cpi_dec2025": float(cpi_records[0]["cpi_index"]) if cpi_records else 124.23,
    ...
    return {
        ...
        "mospi_benchmark": {
            "latest_available_index": float(cpi_records[0]["cpi_index"]) if cpi_records else 124.23,
            "latest_available_period": f"{cpi_records[0].get('month')} {cpi_records[0].get('year')}" if cpi_records else "Dec 2025"
        },
        "calibration_metrics": {
